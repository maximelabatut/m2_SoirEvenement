...
public class AuthentificationController {
    ...
    public ResponseEntity inscription(@Valid @RequestBody(required = true) Login login) throws Exception{
        ...
        try {
            ...
            // Insertion de l'utilisateur dans le service utilisateur
            HttpClient httpClient = HttpClientBuilder.create().build();
            String url = "http://localhost:8080/utilisateur/";
            HttpPost request = new HttpPost(url);
            StringEntity params =new StringEntity("{\"pseudo\":\""+login.getPseudo()+"\"}");
